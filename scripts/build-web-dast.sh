#!/usr/bin/env bash
# Build ao-web-dast reproducibly, for the Linux architectures a skill
...
# embedded into the daemon by `go build -tags ao_embed_web_dast`.
#
...
pkg_dir="${backend_dir}/internal/webdast/webdastbin"
out_dir="${pkg_dir}/artifacts"
provenance="${pkg_dir}/provenance.json"

SOURCE_PKG="./cmd/ao-web-dast"
...
if [[ "${mode}" == "clean" ]]; then
  rm -f "${out_dir}"/ao-web-dast-linux-*
