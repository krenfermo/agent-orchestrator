...
// WorkflowPlanStatus is where a run's plan is in its life: generated by the
// planner, checked by validation, and accepted or refused by the approver.
type WorkflowPlanStatus string

// The plan lifecycle, in order. Validated is the planner's output having passed
// structural validation; approved is the separate decision that it may be
// executed. Invalid and rejected are the two ways a plan ends without ever
// becoming work: the machine refused it, or a person did.
const (
...
// WorkflowPlanApprovalMode says who is expected to approve a validated plan.
type WorkflowPlanApprovalMode string

// Manual parks a validated plan until a person approves it; auto approves it on
// validation. The mode is recorded on the plan rather than read from
// configuration at approval time, so a plan is always approved under the policy
...
// WorkflowPlanCommandStatus tracks the planner invocation itself, separately
// from the plan it produces: it is how a run tells "no plan yet because the
// planner has not been asked" from "no plan because the planner failed".
type WorkflowPlanCommandStatus string

// The planner command lifecycle. Responded means output came back and is being
// parsed; completed means that output became a plan record. The two are split so
// a crash between them is recoverable without re-running the planner.
const (
...
// WorkflowPlanRecord is one run's plan row: the planner's output, the context it
// was generated from, and the decisions taken about it. PlanHash pins the
// generated plan so a later approval can prove it approved this plan and not a
// regenerated one.
...
// the task's own view, distinct from the state of the child run executing it.
type WorkflowTaskState string

// The task lifecycle. Blocked and eligible are the two pre-execution states
// (dependencies outstanding, or ready to dispatch); the rest are described
// individually below where the distinction is not obvious.
const (
...
// WorkflowTask is one unit of planned work: what to do, how to tell it is done,
// what it may touch, and what it is waiting on.
...
// WorkflowTaskWaitingReason says why an otherwise-ready task has not been
// dispatched, so a stalled plan can be read without re-deriving the schedule.
type WorkflowTaskWaitingReason string

// The two reasons a task waits: an unmet dependency edge, or an estimated write
// region it shares with a sibling that is currently running.
const (
