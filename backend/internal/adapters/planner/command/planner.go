...
%s`, req.MaxSteps, req.Objective, string(contextJSON))
	schema := fmt.Sprintf(`{"type":"object","additionalProperties":false,"required":["version","objective","summary","steps"],"properties":{"version":{"const":"v1"},"objective":{"type":"string"},"summary":{"type":"string"},"steps":{"type":"array","minItems":1,"maxItems":%d,"items":{"type":"object","additionalProperties":false,"required":["id","title","description","dependencies","acceptanceCriteria","verify"],"properties":{"id":{"type":"string"},"title":{"type":"string"},"description":{"type":"string"},"dependencies":{"type":"array","items":{"type":"string"}},"acceptanceCriteria":{"type":"array","minItems":1,"items":{"type":"string"}},"files":{"type":"array","items":{"type":"string"}},"packages":{"type":"array","items":{"type":"string"}},"verify":{"type":"object","additionalProperties":false,"required":["commands","files"],"properties":{"commands":{"type":"array","items":{"type":"object","additionalProperties":false,"required":["command","args","workingDirectory","timeoutSeconds","requiredExitCode","retrySafe"],"properties":{"command":{"type":"string"},"args":{"type":"array","items":{"type":"string"}},"workingDirectory":{"type":"string"},"timeoutSeconds":{"type":"integer","minimum":1,"maximum":3600},"requiredExitCode":{"type":"integer"},"retrySafe":{"type":"boolean"}}}},"files":{"type":"array","items":{"type":"object","additionalProperties":false,"required":["path","exists"],"properties":{"path":{"type":"string"},"exists":{"type":"boolean"},"exactContent":{"type":"string"},"sha256":{"type":"string"}}}}}}}}}}}`, req.MaxSteps)
	model := p.Model
	if model == "" {
		model = "sonnet"
	}
