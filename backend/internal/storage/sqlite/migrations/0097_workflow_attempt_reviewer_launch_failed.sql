...
-- Checkpoint 8C: Automatic Claude Reviewer.
--
-- Same table-rebuild dance as 0096 (SQLite cannot widen a CHECK constraint in
-- place). Adds exactly one new error_class value the review step's dispatch
-- failure path needs: reviewer_launch_failed, distinct from 8B's
-- session_create_failed/agent_start_failed (which name a *worker* Codex
-- session failure) because this one names a *reviewer* pane launch failure.
