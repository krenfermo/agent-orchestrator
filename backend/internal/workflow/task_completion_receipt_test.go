	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			without := evaluateWorkStepProgress(
				true, session(tc.state, tc.term, false), tc.available, tc.obs, "base", now, dispatchedAt,
			)
			with := evaluateWorkStepProgress(
				true, session(tc.state, tc.term, true), tc.available, tc.obs, "base", now, dispatchedAt,
			)
			if with != without {
