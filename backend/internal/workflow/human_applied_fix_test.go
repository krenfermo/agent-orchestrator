...
func newBudgetExhaustedFixture(t *testing.T) *budgetExhaustedFixture {
	...
	f.parkFixAndReviewAsStop(workflowcore.ReasonFixBudgetExhausted,
...
func TestHumanAppliedFixOnlyAppliesToABudgetExhaustedStop(t *testing.T) {
	...
	f.parkFixAndReviewAsStop(workflowcore.ReasonFixWorkerBlocked, "the fix worker is waiting on input")
...
func TestHumanAppliedFixRecoveriesAreBounded(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		...
		f.parkFixAndReviewAsStop(workflowcore.ReasonFixBudgetExhausted, "the reviewer still requests changes")
		f.mutateSession(func(rec *domain.SessionRecord) {
			rec.Activity = domain.Activity{State: domain.ActivityIdle, LastActivityAt: f.clk.Now().Add(-time.Hour)}
			rec.TurnCompletedAt = f.clk.Now().Add(-time.Hour)
		})
...
// parkFixAndReviewAsStop writes the durable shape a fix stop leaves behind: the fix and
...
func (f *fixRecoveryFixture) parkFixAndReviewAsStop(reason, detail string) {
	f.t.Helper()
